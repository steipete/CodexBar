...
test('family representatives still lead, so the summary and tray report the binding pools', () => {
    ...
    const rows = model.rows(JSON.stringify([{provider: 'antigravity', usage: {
        primary: geminiWeekly, secondary: claudeSession, extraRateWindows: [
            ...
            quotaSummary('gemini-weekly', 'Gemini weekly', geminiWeekly),
            quotaSummary('claude-5h', 'Claude/GPT 5-hour', claudeSession)]}}]));
    // The tooltip names the lane the bar leads with: the session, bound by the exhausted Claude/GPT pool.
    assert.equal(model.summary(rows, 'remaining'), 'antigravity 0%');
    assert.equal(model.barLabel(rows, 'remaining'), '5H 0% · 7D 5%');
...
test('a provider whose only quota is cadence-less still reports it in the bar and the tooltip', () => {
    ...
    assert.equal(model.barLabel(rows, 'remaining'), 'Model A 36%');
    ...
    const detailOnly = model.rows(JSON.stringify([{provider: 'antigravity', usage: {extraRateWindows: [
        {id: 'experimental', title: 'Experimental Model', usageKnown: true,
            window: {usedPercent: 64, windowMinutes: null}}]}}]));
    assert.equal(model.barLabel(detailOnly, 'remaining'), '—');
    // Detail extras crowding the limit cannot trim the marked fallback away.
    const crowded = model.rows(JSON.stringify([{provider: 'antigravity', usage: {extraRateWindows: [
        ...Array.from({length: 8}, (_, index) => ({id: 'image-' + index, title: 'Image ' + index, usageKnown: true,
            window: {usedPercent: 99, windowMinutes: null}})),
        {id: 'antigravity-compact-fallback-model-a', title: 'Model A', usageKnown: true,
            window: {usedPercent: 64, windowMinutes: null}}]}}]));
    assert.equal(model.summary(crowded, 'remaining'), 'antigravity 36%');
});

test('a crowded cadence cannot push another cadence out of the extra limit', () => {
    const session = (id, used) => ({id: 'antigravity-quota-summary-' + id, title: 'Pool ' + id, usageKnown: true,
        window: {usedPercent: used, windowMinutes: 300, resetsAt: '2030-01-01T05:00:00Z'}});
    const weekly = {id: 'antigravity-quota-summary-weekly', title: 'Weekly pool', usageKnown: true,
        window: {usedPercent: 40, windowMinutes: 10080, resetsAt: '2030-01-07T00:00:00Z'}};
    const extras = Array.from({length: 10}, (_, index) => session('s' + index, 50 + index));
    const [row] = model.rows(JSON.stringify([{provider: 'antigravity', usage: {extraRateWindows: [...extras, weekly]}}]));
    assert.equal(row.windows.length, 8);
    assert.ok(row.windows.some(window => window.label === 'Weekly pool'));
    assert.ok(row.windows.some(window => window.label === 'Pool s9'), 'the tightest session pool survives');
    assert.equal(model.barLabel([row], 'remaining'), '5H 41% · 7D 60%');
});
...
test('the bar limit is display only: it counts the providers it hides', () => {
    const rows = model.rows(JSON.stringify(['codex', 'claude', 'gemini', 'zai'].map(provider => (
        {provider, usage: {secondary: weekly}}))));
    assert.equal(model.barLabel(rows, 'remaining', {maxProviders: 2}), 'CX 7D 61%  ·  CL 7D 61%  +2');
    assert.equal(model.barLabel(rows, 'remaining', {maxProviders: 0}).split('  ·  ').length, 4);
    assert.equal(model.barLabel(rows.slice(0, 1), 'remaining', {maxProviders: 2}), '7D 61%');
...
test('the label and the per-provider entries show the same providers, in the same order', () => {
    ...
    // The display limit trims both views alike, and the label counts what it hides.
    const limited = model.barSegments(rows, 'remaining', {maxProviders: 2});
    assert.deepEqual([...limited.map(segment => segment.provider)], ['codex', 'claude']);
    assert.equal(model.barLabel(rows, 'remaining', {maxProviders: 2}), 'CX 5H 90%  ·  CL 5H 80%  +1');
});
