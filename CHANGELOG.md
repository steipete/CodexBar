...
- Mistral: fall back to the tRPC procedures behind the Admin usage page when the legacy billing endpoint fails (it now answers HTTP 500 for valid sessions), showing that source's list-price total as Consumption rather than API spend; match unit prices by event type so token prices are no longer overwritten by audio-second prices of the same model, and treat login redirects as an expired session so the next browser is tried. Thanks @welcoMattic!
