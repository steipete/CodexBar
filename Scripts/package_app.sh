...
patch_keyboard_shortcuts() {
  ...
  chmod +w "$util_path" "$recorder_path" 2>/dev/null || true
  python3 - "$util_path" "$recorder_path" <<'PY'
import sys
import re
from pathlib import Path

util_path = Path(sys.argv[1])
recorder_path = Path(sys.argv[2])

util_text = util_path.read_text()
if ".keyboardShortcutsSafeBundle" not in util_text:
...
"""
    ...
    marker = "}\n\n\nextension Data {"
    if marker not in util_text:
        raise SystemExit("Marker not found in Utilities.swift; patch failed.")

    util_text = util_text.replace(marker, "}\n\n" + inject + "\n\nextension Data {")
    util_path.write_text(util_text)

if recorder_path.exists():
    recorder_text = recorder_path.read_text()
    preview_pattern = re.compile(r'\n#Preview\s*\{.*?\n\}\n', re.DOTALL)
    patched_recorder = preview_pattern.sub('\n', recorder_text)
    if patched_recorder != recorder_text:
        recorder_path.write_text(patched_recorder)
PY
}
