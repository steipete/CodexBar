...
- `Scripts/pull_build_install_and_run.sh`: runs `git pull --ff-only` and invokes `build_install_and_run.sh` unless the recorded commit/configuration still matches the installed bundle's signing mode, identity/team, signed identifier, and embedded `CodexGitCommit`. Failed, deleted, differently signed, or externally replaced installs are retried instead of being skipped.
