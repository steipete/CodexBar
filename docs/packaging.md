...
For a local run, set `CODEXBAR_CCUSAGE_PATH` to a vetted executable. For a packaged app, set `CODEXBAR_CCUSAGE_SOURCE` while running `Scripts/package_app.sh`; the helper must contain every architecture in `ARCHES` and is copied to `Contents/Helpers/ccusage`. Set `CODEXBAR_REQUIRE_CCUSAGE=1` to make packaging fail when the source is missing. The fallback is best-effort: missing helpers, timeouts, invalid JSON, and non-zero exits retain the native result.
