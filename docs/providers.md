...
| Claude | Admin API key (`api`) when configured; otherwise App Auto: CLI PTY (`claude`) → Web API (`web`). CLI Auto: Web API (`web`) → CLI PTY (`claude`). Explicit OAuth sources and selected OAuth token accounts route directly to OAuth. |
...
- Explicit app OAuth and selected OAuth token accounts route directly to OAuth. App OAuth can use environment,
  secure-storage-file, or CodexBar-owned credentials and never reads Claude Code's foreign Keychain item.
