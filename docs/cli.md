...
- `codexbar cost` prints token cost usage for Claude, Codex, Cursor, and Antigravity.
  ...
  - Antigravity reads supported local token history: history scanning uses no web, provider CLI, or credential access. Token counts map to API-equivalent dollar estimates (not billed) through the on-disk models.dev catalog: models absent from the catalog stay cost-less with tokens intact and coverage disclosed, while unsupported timestamps and incomplete histories remain unavailable (see `docs/antigravity.md`). Pricing reuses the cached catalog within its TTL (same policy as Codex/Claude) and refreshes it in the foreground when stale or missing; refresh failures keep the existing cache. The same provider selection applies to `serve /cost` and dashboard cost collection.
    Text output labels this as an API-equivalent estimate and distinguishes unavailable or incomplete history from a complete period with no recorded usage.
