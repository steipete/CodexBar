...
- `record-manifest.json` records the first packaged process and its 30-day ledger hash.
- `reload-manifest.json` proves the second packaged process opened that exact ledger hash and rendered 60 tracked days.
...
success it requires exactly two PNGs, both manifests, and a manifest privacy scan. The tracked manifests omit the temporary
