...
- Key: `SUPublicEDKey` is sourced from the fork’s Sparkle keypair. Keep the Ed25519 private key safe; use it when generating the appcast.
...
- Channel model: CodexBar uses a single Sparkle update stream. The About pane no longer exposes stable/beta selection, and Sparkle always reads the default channel.
