...
2. **Transcript/session correlation**
   ...
   - OhMyPi: resolve session roots for the effective active profile only. `OMP_PROFILE` takes precedence over `PI_PROFILE`; the default uses `PI_CONFIG_DIR` or `~/.omp`, with the configured agent root and supported XDG fallback. Named profile roots are constrained to the profile directory and invalid profile values fail closed. Do not scan other profiles. For a matched live `omp` process, read only bounded JSONL metadata from its candidate root: an optional first `title` record followed by a `session` header. Use only the session id, cwd, timestamp, and a sanitized bounded title. Correlate by cwd; an unmatched live process uses the `pid:<pid>` fallback. An OhMyPi file, breadcrumb, or stale record without a matched live process never creates an AgentSession.
   ...
   - Never read whole Codex, Claude Code, or OhMyPi transcripts. Directory enumeration, file metadata, and title/session-header parsing are bounded by the scanner budget and entry/record limits. Future timestamps are clamped to the scan time.

The scanner is `Sendable`; ps/lsof parsing and session-header parsing are dedicated pure parser types fed by strings or bounded fixture data so tests do not need live processes. Process command lines and cwd values are sensitive metadata and are retained only long enough to correlate a row under the privacy rules below.
...
Project labels remain the default because thread/session titles, cwd values, and process command lines can contain sensitive text. Descriptive labels are opt-in. CodexBar reads only bounded metadata needed for correlation and display; it does not load prompts, tool payloads, or whole transcripts, does not modify provider state, and does not persist title text separately. OhMyPi metadata is read only under the active profile root and only contributes to a row associated with a live process. Remote discovery and SSH fetching occur only while Agent Sessions is enabled.
...
- OhMyPi active-profile root resolution, bounded title/session-header parsing, cwd correlation, and the no-file-only rule.
