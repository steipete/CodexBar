...
- `MiniMaxModelUsage.identifier` is collision-safe for UI diffing: rows with duplicated/missing `model_name`/`model_type` still get distinct stable IDs within a payload (used by SwiftUI `ForEach`).
