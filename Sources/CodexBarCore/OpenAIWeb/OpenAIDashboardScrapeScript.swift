    ...
    (() => {
      ...
      const nearestChartTitleTextForRoot = (root) => {
        ...
        try {
          ...
          const titleLike = titleLikeElements(document);
          let nearest = null;
          for (const titleNode of titleLike) {
      ...
      const usageBreakdownJSON = (() => {
        try {
          ...
          const eligibleCandidates = titledCandidates.length > 0
            ? titledCandidates
            : (unknownTitleCandidates.length === 1 && candidates.length === 1 ? unknownTitleCandidates : []);
          ...
          if (eligibleCandidates.length === 0 && candidates.length > 0) {
            if (unknownTitleCandidates.length > 0) {
              debug.error = 'No usage breakdown chart title found. Candidate titles: ' +
