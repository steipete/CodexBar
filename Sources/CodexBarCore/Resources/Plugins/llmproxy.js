  ...
  async fetchUsage(ctx) {
    ...
    const providers = Object.entries(object(root.providers))
      .map(([name, value]) => {
        ...
        credentials += _nullishCoalesce(number(stats.credential_count, true), () => 0);
        active += _nullishCoalesce(number(stats.active_count, true), () => 0);
        ...
        return {
          name,
          requests: _nullishCoalesce(number(stats.total_requests, true), () => 0),
          tokens:
            _nullishCoalesce(number(tokens.input_cached, true), () => 0) +
            _nullishCoalesce(number(tokens.input_uncached, true), () => 0) +
            _nullishCoalesce(number(tokens.output, true), () => 0),
          cost: number(stats.approx_cost),
        };
      })
      .sort((a, b) => b.requests - a.requests || (a.name < b.name ? -1 : a.name > b.name ? 1 : 0));
    const requests = _nullishCoalesce(number(summary.total_requests, true), () =>
      providers.reduce((sum, p) => sum + p.requests, 0),
    );
    const tokens = _nullishCoalesce(number(summary.total_tokens, true), () =>
      providers.reduce((sum, p) => sum + p.tokens, 0),
    );
    const sum = providers.reduce((sum, p) => sum + _nullishCoalesce(p.cost, () => 0), 0);
    const cost = _nullishCoalesce(number(summary.approx_cost), () => (sum > 0 ? sum : undefined));
