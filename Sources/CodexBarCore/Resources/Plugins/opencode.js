...
const extractServerErrorMessage = (text) => {
...
const isExplicitNullPayload = (text) => {
...
const billingDoubleValue = (value) => {
...
const normalizeWorkspaceID = (raw) => {
  const trimmed = _nullishCoalesce(raw, () => "").trim();
...
const parseWorkspaceIDsFromJSON = (text) => {
  const object = parseJSON(text);
...
const parseWindow = (ctx, dict, now) => {
  ...
  return {
    ...
    resetInSec: Math.max(
      0,
      _nullishCoalesce(resetInSec, () => 0),
    ),
  };
};

const collectWindowCandidates = (ctx, object, now) => {
  const candidates = [];
...
const pickCandidateFrom = (candidates, pickShorter) => {
  let best;
...
const pickCandidate = (preferred, fallback, pickShorter, excluding) =>
  _nullishCoalesce(
    pickCandidateFrom(
...
const buildSnapshot = (ctx, rolling, weekly, now, renewsAt) => {
...
const parseUsageDictionary = (ctx, dict, now, inheritedRenewsAt) => {
  const renewsAt = _nullishCoalesce(dateValue(ctx, valueForKeys(dict, RENEW_AT_KEYS)), () => inheritedRenewsAt);
...
const parseUsageNested = (ctx, dict, now, depth, inheritedRenewsAt) => {
...
const parseUsageFromCandidates = (ctx, object, now, inheritedRenewsAt) => {
  ...
  const weekly = pickCandidate(
    ...
    candidates,
    false,
    _optionalChain([rolling, "optionalAccess", (_) => _.id]),
  );
  if (!rolling || !weekly) return null;
  const renewsAt = _nullishCoalesce(
    isObject(object) ? dateValue(ctx, valueForKeys(object, RENEW_AT_KEYS)) : undefined,
    () => inheritedRenewsAt,
  );
  return {
...
const parseUsageJSON = (ctx, object, now) => {
...
const parseSubscriptionJSON = (ctx, text, now) => {
  const object = parseJSON(text);
...
const extractDouble = (pattern, text) => {
...
const parseSubscription = (ctx, text, now) => {
...
const findCustomerDictionary = (object) => {
  if (isObject(object)) {
...
const billingFieldNumber = (field, text) => {
...
const parseBillingPayload = (text) => {
...
const parseBilling = (text) => _nullishCoalesce(parseBillingJSON(text), () => parseBillingPayload(text));
...
const nextServerInstance = () => `server-fn:${(serverInstanceCounter += 1)}-${Date.now().toString(36)}`;

const fetchServerText = async (ctx, request, cookieHeader) => {
  const headers = {
    Cookie: cookieHeader,
  ...
  try {
    response =
      request.method === "GET"
        ...
        : await ctx.http.post(url, { headers, timeoutSeconds, body: _nullishCoalesce(request.args, () => []) });
  } catch (error) {
    const transport = error;
    if (transport.transportClass === "cancelled") throw error;
    throw fetchError(NETWORK, `OpenCode network error: ${_nullishCoalesce(transport.message, () => "request failed")}`);
  }
  const bodyText = _nullishCoalesce(response.bodyText, () => "");
...
const fetchWorkspaceID = async (ctx, cookieHeader) => {
...
const missingSubscriptionDataError = (workspaceID) =>
  fetchError(
    API,
...
const fetchSubscriptionInfo = async (ctx, workspaceID, cookieHeader, now) => {
...
const fetchPayAsYouGoUsage = async (ctx, workspaceID, cookieHeader) => {
  const text = await fetchServerText(
    ctx,
    {
  ...
  return {
    ...
    cost: {
      ...
      limit: _nullishCoalesce(billing.monthlyLimitUSD, () => 0),
      currency: "USD",
      period: "Monthly",
      balance: _nullishCoalesce(billing.balanceUSD, () => null),
    },
  };
};

const fetchUsageForSession = async (ctx, cookieHeader, workspaceIDOverride) => {
  const now = ctx.date.now();
  const workspaceID = await _asyncNullishCoalesce(
    workspaceIDOverride,
    async () => await fetchWorkspaceID(ctx, cookieHeader),
  );
  try {
    const text = await fetchSubscriptionInfo(ctx, workspaceID, cookieHeader, now);
    const subscription = parseSubscription(ctx, text, now);
    const resetsAt = (seconds) => {
    ...
    try {
      ...
      if (billingError.kind === INVALID) throw billingError;
      if (billingError.transportClass === "cancelled") throw billingError;
      ctx.log(`OpenCode billing fallback failed: ${_nullishCoalesce(billingError.message, () => billingError)}`);
    }
    throw error;
  }
};

const classify = (ctx, error) => {
  const failure = error;
  switch (failure.kind) {
    case INVALID:
