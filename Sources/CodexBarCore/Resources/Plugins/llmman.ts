defineProvider({
  ...
  async fetchUsage(ctx) {
    ...
    const models = (value: unknown): Array<[string, number]> => {
      if (!value || typeof value !== "object" || Array.isArray(value)) return fail();
      return Object.entries(value as Record<string, unknown>)
        .map(([name, size]): [string, number] => [name, bytes(size)])
    ...
    const total = (list: Array<[string, number]>) => list.reduce((sum, [, size]) => sum + size, 0);
    ...
    const count = (list: Array<[string, number]>) => `${list.length} · ${size(total(list))}`;

    // Best effort: the version is display-only, and older daemons may answer differently.
    let version: string | undefined;
    try {
      const reply = await get("/api/version");
      const value = reply.status === 200 ? JSON.parse(reply.bodyText)?.version : undefined;
