...
function parseEnvelope(bodyText: string, ctx: CodexBarPluginContext, label: string): unknown {
  let payload: ClineEnvelope;
  try {
    payload = JSON.parse(bodyText) as ClineEnvelope;
  } catch (error) {
    void error;
...
function failAuth(ctx: CodexBarPluginContext): never {
...
async function getJSON(
  ...
  try {
    ...
    throw ctx.fail.networkFailure(`Cline network error: ${(error as Error)?.message || String(error)}`);
  }
...
defineProvider({
  ...
  async fetchUsage(ctx) {
    ...
    const meData = parseEnvelope(meResponse.bodyText, ctx, "account") as ClineUser;
    ...
    const balanceData = parseEnvelope(balanceResponse.bodyText, ctx, "balance") as ClineBalance;
