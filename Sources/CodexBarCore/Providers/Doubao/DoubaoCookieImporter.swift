...
public enum DoubaoCookieImporter {
    private static let log = CodexBarLog.logger(LogCategories.doubaoCookie)
    private static let cookieClient = BrowserCookieClient()
    private static let cookieDomains = ["console.volcengine.com"]
    private static let cookieImportOrder: BrowserCookieImportOrder =
        ProviderDefaults.metadata[.doubao]?.browserCookieOrder ?? Browser.defaultImportOrder
    ...
    public struct SessionInfo: Sendable {
        ...
        public var cookieHeader: String? {
            let parts = self.cookies.compactMap { cookie -> String? in
        ...
        logger: ((String) -> Void)? = nil) throws -> [SessionInfo]
    {
        ...
        guard !sessions.isEmpty else {
            throw DoubaoCookieImportError.noCookies
        }
        return sessions
    }
        ...
        logger: ((String) -> Void)? = nil) throws -> [SessionInfo]
    {
        ...
        for group in sortedGroups where !group.isEmpty {
            ...
            log("Found \(httpCookies.count) cookies for console.volcengine.com in \(label)")
    ...
    private static func mergedLabel(for sources: [BrowserCookieStoreRecords]) -> String {
        ...
        if base.hasSuffix(" (Network)") { return String(base.dropLast(" (Network)".count)) }
        return base
    }
...
enum DoubaoCookieImportError: LocalizedError {
    case noCookies

    var errorDescription: String? {
        switch self {
        case .noCookies: "No Doubao session cookies found in browsers."
        }
    }
}
#endif
