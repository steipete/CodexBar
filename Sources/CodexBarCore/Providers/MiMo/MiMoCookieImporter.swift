...
public enum MiMoCookieImporter {
        ...
        logger: ((String) -> Void)? = nil) throws -> [SessionInfo]
    {
        ...
        guard !sessions.isEmpty else {
            throw MiMoCookieImportError.noCookies
        }
        return sessions
    }
        ...
        logger: ((String) -> Void)? = nil) throws -> [SessionInfo]
    {
        ...
        for group in sortedGroups where !group.isEmpty {
            ...
            log("Found \(httpCookies.count) cookies for platform.xiaomimimo.com in \(label)")
...
enum MiMoCookieImportError: LocalizedError {
    case noCookies

    var errorDescription: String? {
        switch self {
        case .noCookies:
            "No MiMo session cookies found in browsers."
        }
    }
}
#endif
