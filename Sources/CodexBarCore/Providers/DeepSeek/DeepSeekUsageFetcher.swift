...
public struct DeepSeekUsageFetcher: Sendable {
        ...
        async throws -> DeepSeekUsageSnapshot
    {
        ...
        let balanceData = try await fetchBalanceData(apiKey)
        var snapshot = try Self.parseSnapshot(data: balanceData)

        if let summaryTask {
            let summary = await self.completedOptionalUsageSummary(
                from: summaryTask,
    ...
    private static func completedOptionalUsageSummary(
        ...
        joinGrace: Duration) async -> DeepSeekUsageSummary?
    {
        do {
    ...
    public static func fetchUsageSummary(
        apiKey: String,
        now: Date = Date(),
        calendar: Calendar = .current) async throws -> DeepSeekUsageSummary
    {
        ...
        let amountData = try await self.fetchAmount(apiKey: apiKey, month: month, year: year)
        let costData = try await self.fetchCost(apiKey: apiKey, month: month, year: year)
