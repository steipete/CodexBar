...
/// Optional `/activity` enrichment. Any endpoint, transport, or decoding failure leaves the
/// already-valid credits/key snapshot intact; explicit task cancellation still propagates.
public enum OpenRouterActivityUsageFetcher {
    private static let timeoutSeconds: TimeInterval = 5

    public static func fetchOptional(
        apiKey: String,
        baseURL: URL,
        transport: any ProviderHTTPTransport,
        now: Date = Date(),
        historyDays: Int = 30) async throws -> OpenRouterActivityUsageSnapshot?
    {
        guard !apiKey.isEmpty else { return nil }
        let sourceTask = Task<OpenRouterActivityUsageSnapshot?, Error> {
            var request = URLRequest(url: baseURL.appendingPathComponent("activity"))
            ...
            do {
                ...
                return try OpenRouterActivityUsageSnapshot(
                    data: response.data,
                    now: now,
        ...
        switch await race.value(joinGrace: .seconds(Self.timeoutSeconds)) {
