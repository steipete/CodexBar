...
public enum ZhipuCookieImporter {
    private static let log = CodexBarLog.logger(LogCategories.zhipuCookie)
    private static let cookieClient = BrowserCookieClient()
    private static let cookieDomains = ["open.bigmodel.cn"]
    private static let cookieImportOrder: BrowserCookieImportOrder =
        ProviderDefaults.metadata[.zhipu]?.browserCookieOrder ?? Browser.defaultImportOrder
        ...
        logger: ((String) -> Void)? = nil) throws -> [SessionInfo]
    {
        ...
        for group in sortedGroups where !group.isEmpty {
            ...
            log("Found \(httpCookies.count) cookies for open.bigmodel.cn in \(label)")
...
enum ZhipuCookieImportError: LocalizedError {
    case noCookies

    var errorDescription: String? {
        switch self {
        case .noCookies:
            "No Zhipu session cookies found in browsers."
        }
    }
}
#endif
