...
public enum CodexOpenAIWorkspaceResolver {
        ...
        session: URLSession = .shared) async throws -> CodexOpenAIWorkspaceIdentity?
    {
        ...
        if let account = decoded.items.first(where: {
            ...
            let resolvedLabel = self.resolveWorkspaceLabel(from: account)
    ...
    public static func mergeAuthoritativeIdentity(
        into accountInfo: AccountInfo,
        authoritativeIdentity: CodexOpenAIWorkspaceIdentity?) -> AccountInfo
    {
        guard let authoritativeIdentity else { return accountInfo }
        return AccountInfo(
            email: accountInfo.email,
            plan: accountInfo.plan,
            workspaceLabel: authoritativeIdentity.workspaceLabel,
            workspaceAccountID: authoritativeIdentity.workspaceAccountID)
    }
