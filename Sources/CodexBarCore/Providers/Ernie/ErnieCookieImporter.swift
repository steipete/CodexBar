...
public enum ErnieCookieImporter {
    private static let log = CodexBarLog.logger(LogCategories.ernieCookie)
    private static let cookieClient = BrowserCookieClient()
    private static let cookieDomains = ["console.bce.baidu.com"]
    private static let cookieImportOrder: BrowserCookieImportOrder =
        ProviderDefaults.metadata[.ernie]?.browserCookieOrder ?? Browser.defaultImportOrder
        ...
        logger: ((String) -> Void)? = nil) throws -> [SessionInfo]
    {
        ...
        guard !sessions.isEmpty else {
            throw ErnieCookieImportError.noCookies
        }
        return sessions
    }
        ...
        logger: ((String) -> Void)? = nil) throws -> [SessionInfo]
    {
        ...
        for group in sortedGroups where !group.isEmpty {
            ...
            log("Found \(httpCookies.count) cookies for console.bce.baidu.com in \(label)")
...
enum ErnieCookieImportError: LocalizedError {
    case noCookies

    var errorDescription: String? {
        switch self {
        case .noCookies: "No ERNIE session cookies found in browsers."
        }
    }
}
#endif
