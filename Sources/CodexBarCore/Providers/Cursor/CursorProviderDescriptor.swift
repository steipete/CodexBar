...
public enum CursorProviderDescriptor {
    ...
    private static let credentials = ProviderCredentialAdapter(
        ...
        selectedAccountSourceModeResolver: { base, account, _ in
            ...
            base == .oauth && account != nil ? .web : base
        })
        ...
        ProviderDescriptor(
            ...
            cli: ProviderCLIConfig(
                ...
                browserSupportExemption: { sourceMode, _, settings in
                    ...
                    // Auto is always exempt: it can still fetch with the Cursor app token
                    // even when the cookie source is Off.
                    sourceMode == .auto || settings?.cursor?.cookieSource != .off
                    #else
                    false
                    #endif
                }))
...
struct CursorAppTokenFetchStrategy: ProviderFetchStrategy {
    ...
    func isAvailable(_ context: ProviderFetchContext) async -> Bool {
        ...
        if context.sourceMode == .auto,
           CursorStatusProbe.autoModeDefersToExplicitSelection(
               cursorSettings: context.settings?.cursor,
               cachedEntry: self.loadCachedEntry())
        {
            return false
        }
