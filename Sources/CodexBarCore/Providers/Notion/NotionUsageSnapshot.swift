...
public enum NotionUsageError: LocalizedError, Sendable {
...
public struct NotionAccount: Sendable, Equatable {
    ...
    public func resolveWorkspace(preferredID: String? = nil) -> NotionWorkspace? {
        if let preferredID = Self.normalizeSpaceID(preferredID) {
            if let match = self.workspaces.first(where: { Self.normalizeSpaceID($0.id) == preferredID }) {
                return match
            }
            // An id the account cannot see is still worth attempting; the API decides.
            return NotionWorkspace(id: preferredID, name: nil, planType: nil, subscriptionTier: nil)
        }
...
public struct NotionUsageSnapshot: Sendable {
    ...
    public func toUsageSnapshot() -> UsageSnapshot {
        let primary = RateWindow(
            usedPercent: Self.percent(used: self.rateLimit.window?.used, limit: self.rateLimit.window?.limit),
            windowMinutes: Self.minutes(fromWindowToken: self.rateLimit.window?.window),
            resetsAt: Self.rollingReset(from: self.rateLimit.resetsInSeconds, now: self.updatedAt),
            resetDescription: nil)

        let secondary: RateWindow? = self.rateLimit.billingPeriodWindow.map { billing in
            RateWindow(
                usedPercent: Self.percent(used: billing.used, limit: billing.limit),
                windowMinutes: nil,
    ...
    static func percent(used: Double?, limit: Double?) -> Double {
        ...
        guard let limit, limit > 0 else { return max(0, used) }
...
public enum NotionUsageParser {
    public static func parseRateLimitStatus(_ data: Data) throws -> NotionCreditRateLimitStatus {
        do {
            return try JSONDecoder().decode(NotionCreditRateLimitStatus.self, from: data)
        } catch {
    ...
    public static func parseSpaces(_ data: Data) throws -> NotionAccount {
        ...
        guard let userID = root.keys.min(), let container = root[userID] as? [String: Any] else {
            throw NotionUsageError.parseFailed("getSpaces response contains no user.")
        }
