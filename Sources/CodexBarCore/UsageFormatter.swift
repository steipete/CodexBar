...
public enum UsageFormatter {
    public static func usageLine(remaining: Double, used: Double, showUsed: Bool) -> String {
        ...
        let key = showUsed ? "%@ used" : "%@ left"
        return String(format: NSLocalizedString(key, comment: ""), percentText)
    }

    public static func resetCountdownDescription(from date: Date, now: Date = .init()) -> String {
        let seconds = max(0, date.timeIntervalSince(now))
        if seconds < 1 { return NSLocalizedString("now", comment: "") }
        ...
        if days > 0 {
            if hours > 0 { return String(format: NSLocalizedString("in %dd %dh", comment: ""), days, hours) }
            return String(format: NSLocalizedString("in %dd", comment: ""), days)
        }
        if hours > 0 {
            if minutes > 0 { return String(format: NSLocalizedString("in %dh %dm", comment: ""), hours, minutes) }
            return String(format: NSLocalizedString("in %dh", comment: ""), hours)
        }
        return String(format: NSLocalizedString("in %dm", comment: ""), totalMinutes)
    }
        ...
        now: Date = .init()) -> String?
    {
        ...
        if let desc = window.resetDescription {
            ...
            let localizedResets = NSLocalizedString("Resets", comment: "")
            if trimmed.lowercased().hasPrefix("resets") || trimmed.hasPrefix(localizedResets) { return trimmed }
            return String(format: NSLocalizedString("Resets %@", comment: ""), trimmed)
        }
        return nil
    }
    ...
    public static func updatedString(from date: Date, now: Date = .init()) -> String {
        ...
        if abs(delta) < 60 {
            return NSLocalizedString("Updated just now", comment: "")
        }
        if let hours = Calendar.current.dateComponents([.hour], from: date, to: now).hour, hours < 24 {
            ...
            if seconds < 3600 {
                ...
                return String(format: NSLocalizedString("Updated %dm ago", comment: ""), minutes)
            }
    ...
    public static func creditsString(from value: Double) -> String {
        ...
        return String(format: NSLocalizedString("%@ left", comment: ""), formatted)
    }
    ...
    public static func creditEventSummary(_ event: CreditEvent) -> String {
        ...
        return String(
            format: NSLocalizedString("%@ · %@ · %@ credits", comment: ""),
