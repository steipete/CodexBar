...
struct AboutPane: View {
    ...
    var body: some View {
        VStack(spacing: 12) {
            ...
            VStack(spacing: 2) {
                Text(AppStrings.tr("CodexBar"))
                    .font(.title3).bold()
                Text(AppStrings.fmt("Version %@", self.versionString))
                ...
                Text(AppStrings.tr("May your tokens never run out—keep agent limits in view."))
                    .font(.footnote)
                    .foregroundStyle(.secondary)
            }
            ...
            VStack(alignment: .center, spacing: 10) {
                AboutLinkRow(
                    ...
                    title: AppStrings.tr("GitHub"),
                    url: "https://github.com/steipete/CodexBar")
                AboutLinkRow(icon: "globe", title: AppStrings.tr("Website"), url: "https://steipete.me")
                AboutLinkRow(icon: "bird", title: AppStrings.tr("Twitter"), url: "https://twitter.com/steipete")
                AboutLinkRow(icon: "envelope", title: AppStrings.tr("Email"), url: "mailto:peter@steipete.me")
            }
            .padding(.top, 8)
            ...
            if self.updater.isAvailable {
                VStack(spacing: 10) {
                    Toggle(AppStrings.tr("Check for updates automatically"), isOn: self.$autoUpdateEnabled)
                        .toggleStyle(.checkbox)
                        .frame(maxWidth: .infinity, alignment: .center)
                    VStack(spacing: 6) {
                        HStack(spacing: 12) {
                            Text(AppStrings.tr("Update Channel"))
                            Spacer()
                    ...
                    Button(AppStrings.tr("Check for Updates…")) { self.updater.checkForUpdates(nil) }
                }
            } else {
                Text(self.updater.unavailableReason ?? AppStrings.tr("Updates unavailable in this build."))
                    .foregroundStyle(.secondary)
            }

            Text(AppStrings.tr("© 2026 Peter Steinberger. MIT License."))
                .font(.footnote)
                .foregroundStyle(.secondary)
                .padding(.top, 4)
