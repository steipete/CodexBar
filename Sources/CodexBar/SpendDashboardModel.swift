...
struct SpendDashboardModel: Equatable, Sendable {
    ...
    struct DailySummary: Identifiable, Equatable, Sendable {
        ...
        var id: Int {
            Int(self.day.timeIntervalSince1970)
        }
    }
        ...
        calendar: Calendar) -> CurrencyGroup
    {
        ...
        let dailySummaries = Self.dailySummaries(
            summaries: summaries,
            calendar: calendar)
    ...
    private static func hasCompleteRequestHistory(
        ...
        displayCalendar: Calendar) -> Bool
    {
        guard let aggregate = nonnegative(input.snapshot.last30DaysRequests) else { return false }
        let coverage = Self.sourceCoverageInterval(input: input, displayCalendar: displayCalendar)
        var dailyTotal = 0
        for entry in input.snapshot.daily {
            guard let day = Self.day(entry.date, provider: input.provider, displayCalendar: displayCalendar) else {
            ...
            guard coverage.contains(day) else { continue }
            guard let requests = nonnegative(entry.requestCount) else { return false }
        ...
        calendar: Calendar) -> [DailySummary]
    {
        ...
        while day <= coverage.upperBound {
            let providerRows = summaries.compactMap { summary -> DailyProviderRow? in
                let entries = summary.entries.filter { $0.day == day }
                let costs = entries.map { Self.validCost($0.entry.costUSD) }
                guard let totalCost = entries.isEmpty ? 0 : Self.completeCostSum(costs) else { return nil }

                let totalTokens: Int? = if !summary.hasCompleteTokenHistory {
                    nil
                ...
                let requestCount: Int? = if !summary.hasCompleteRequestHistory {
                    nil
                } else if entries.isEmpty {
                    0
                } else {
                    Self.completeIntSum(entries.map { Self.nonnegative($0.entry.requestCount) })
                }
