...
struct DebugPane: View {
    ...
    var body: some View {
        ScrollView(.vertical, showsIndicators: true) {
            VStack(alignment: .leading, spacing: 20) {
                SettingsSection(title: AppStrings.tr("Logging")) {
                    PreferenceToggleRow(
                        title: AppStrings.tr("Enable file logging"),
                        subtitle: AppStrings.fmt("Write logs to %@ for debugging.", self.fileLogPath),
                    ...
                    HStack(alignment: .center, spacing: 12) {
                        VStack(alignment: .leading, spacing: 4) {
                            Text(AppStrings.tr("Verbosity"))
                                .font(.body)
                            Text(AppStrings.tr("Controls how much detail is logged."))
                                .font(.footnote)
                                .foregroundStyle(.tertiary)
                        }
                        Spacer()
                        Picker(AppStrings.tr("Verbosity"), selection: self.$settings.debugLogLevel) {
                    ...
                    Button {
                        ...
                        Label(AppStrings.tr("Open log file"), systemImage: "doc.text.magnifyingglass")
                    }
                    .controlSize(.small)
                }
                ...
                SettingsSection {
                    PreferenceToggleRow(
                        title: AppStrings.tr("Force animation on next refresh"),
                        subtitle: AppStrings.tr("Temporarily shows the loading animation after the next refresh."),
                        binding: self.$store.debugForceAnimation)
                }

                SettingsSection(
                    title: AppStrings.tr("Loading animations"),
                    caption: AppStrings
                        .tr("Pick a pattern and replay it in the menu bar. \"Random\" keeps the existing behavior."))
                {
                    Picker(AppStrings.tr("Animation pattern"), selection: self.animationPatternBinding) {
                        Text(AppStrings.tr("Random (default)")).tag(nil as LoadingPattern?)
                    ...
                    Button(AppStrings.tr("Replay selected animation")) {
                ...
                SettingsSection(
                    title: AppStrings.tr("Probe logs"),
                    caption: AppStrings.tr("Fetch the latest probe output for debugging; Copy keeps the full text."))
                {
                    Picker(AppStrings.tr("Provider"), selection: self.$currentLogProvider) {
                    ...
                    Button {
                        ...
                        Label(AppStrings.tr("Re-run provider autodetect"), systemImage: "dot.radiowaves.left.and.right")
                    }
                    .controlSize(.small)
                ...
                SettingsSection(
                    title: AppStrings.tr("Fetch strategy attempts"),
                    caption: AppStrings.tr("Last fetch pipeline decisions and errors for a provider."))
                {
                    Picker(AppStrings.tr("Provider"), selection: self.$currentFetchProvider) {
                ...
                if !self.settings.debugDisableKeychainAccess {
                    SettingsSection(
                        ...
                        caption: AppStrings
                            .tr("Cookie import + WebKit scrape logs from the last OpenAI cookies attempt."))
                    {
                        HStack(spacing: 12) {
                            Button {
                        ...
                        ScrollView {
                            Text(
                                self.store.openAIDashboardCookieImportDebugLog?.isEmpty == false
                                    ...
                                    : AppStrings
                                    .tr("No log yet. Update OpenAI cookies in Providers → Codex to run an import."))
                                .font(.system(.footnote, design: .monospaced))
                                .textSelection(.enabled)
                                .frame(maxWidth: .infinity, alignment: .leading)
                                .padding(8)
                ...
                SettingsSection(
                    title: AppStrings.tr("Caches"),
                    caption: AppStrings.tr("Clear cached cost scan results."))
                {
                    ...
                    HStack(spacing: 12) {
                        Button {
                            ...
                            Label(AppStrings.tr("Clear cost cache"), systemImage: "trash")
                        }
                ...
                SettingsSection(
                    ...
                    caption: AppStrings
                        .tr("Trigger test notifications for the 5-hour session window (depleted/restored)."))
                {
                    Picker(AppStrings.tr("Provider"), selection: self.$currentLogProvider) {
                        Text("Codex").tag(UsageProvider.codex)
                        Text("Claude").tag(UsageProvider.claude)
                    ...
                    HStack(spacing: 12) {
                        Button {
                            ...
                            Label(AppStrings.tr("Post depleted"), systemImage: "bell.badge")
                        }
                        .controlSize(.small)
                        ...
                        Button {
                            ...
                            Label(AppStrings.tr("Post restored"), systemImage: "bell")
                        }
                        .controlSize(.small)
                    }
                }
                ...
                SettingsSection(
                    title: AppStrings.tr("CLI sessions"),
                    caption: AppStrings
                        .tr("Keep Codex/Claude CLI sessions alive after a probe. Default exits once data is captured."))
                {
                    PreferenceToggleRow(
                        title: AppStrings.tr("Keep CLI sessions alive"),
                        subtitle: AppStrings.tr("Skip teardown between probes (debug-only)."),
                    ...
                    Button {
                        ...
                        Label(AppStrings.tr("Reset CLI sessions"), systemImage: "arrow.counterclockwise")
                    }
                    .controlSize(.small)
                }
                ...
                SettingsSection(
                    ...
                    caption: AppStrings.tr("Inject a fake error message into the menu card for layout testing."))
                {
                    ...
                    TextField(AppStrings.tr("Simulated error text"), text: self.$simulatedErrorText, axis: .vertical)
                        .lineLimit(4)
                    ...
                    HStack(spacing: 12) {
                        ...
                        Button {
                            ...
                            Label(AppStrings.tr("Clear menu error"), systemImage: "xmark.circle")
                        }
                        .controlSize(.small)
                    }
                    ...
                    HStack(spacing: 12) {
                        Button {
                            ...
                            Label(AppStrings.tr("Set cost error"), systemImage: "banknote")
                        }
                        .controlSize(.small)
                        ...
                        Button {
                            ...
                            Label(AppStrings.tr("Clear cost error"), systemImage: "xmark.circle")
                        }
                        .controlSize(.small)
                        .disabled(!supportsTokenError)
                    }
                ...
                SettingsSection(
                    title: AppStrings.tr("CLI paths"),
                    caption: AppStrings
                        .tr("Resolved Codex binary and PATH layers; startup login PATH capture (short timeout)."))
                {
                    self.binaryRow(title: AppStrings.tr("Codex binary"), value: self.store.pathDebugInfo.codexBinary)
                    self.binaryRow(title: AppStrings.tr("Claude binary"), value: self.store.pathDebugInfo.claudeBinary)

                    VStack(alignment: .leading, spacing: 6) {
                        Text(AppStrings.tr("Effective PATH"))
                            .font(.callout.weight(.semibold))
                        ScrollView {
                            Text(
                                self.store.pathDebugInfo.effectivePATH.isEmpty
                                    ? AppStrings.tr("Unavailable")
                    ...
                    if let loginPATH = self.store.pathDebugInfo.loginShellPATH {
                        VStack(alignment: .leading, spacing: 6) {
                            Text(AppStrings.tr("Login shell PATH (startup capture)"))
                                .font(.callout.weight(.semibold))
                            ScrollView {
                                Text(loginPATH)
                                    .font(.system(.footnote, design: .monospaced))
    ...
    private var displayedLog: String {
        if self.logText.isEmpty {
            return self.isLoadingLog ? AppStrings.tr("Loading…") : AppStrings.tr("No log yet. Fetch to load.")
        }
        return self.logText
    }
    ...
    private func binaryRow(title: String, value: String?) -> some View {
        VStack(alignment: .leading, spacing: 6) {
            Text(title)
                .font(.callout.weight(.semibold))
            Text(value ?? AppStrings.tr("Not found"))
                .font(.system(.footnote, design: .monospaced))
    ...
    private func clearCostCache() async {
        ...
        if let error = await self.store.clearCostUsageCache() {
            self.costCacheStatus = AppStrings.fmt("Failed: %@", String(describing: error))
            return
        }

        self.costCacheStatus = AppStrings.tr("Cleared.")
    }

    private func fetchAttemptsText(for provider: UsageProvider) -> String {
        let attempts = self.store.fetchAttempts(for: provider)
        guard !attempts.isEmpty else { return AppStrings.tr("No fetch attempts yet.") }
        return attempts.map { attempt in
            let kind = Self.fetchKindLabel(attempt.kind)
            var line = "\(attempt.strategyID) (\(kind))"
            line += attempt.wasAvailable ? " \(AppStrings.tr("available"))" : " \(AppStrings.tr("unavailable"))"
            if let error = attempt.errorDescription, !error.isEmpty {
                line += AppStrings.fmt(" error=%@", error)
            }
            return line
        }.joined(separator: "\n")
    }
