...
struct ShareStatsCardView: View {
    ...
    private var currencySummary: String {
        ...
        if hiddenCount > 0 {
            return "+\(hiddenCount) more currencies · see subscription rows"
        }
        return "\(self.payload.trackedSourceCount) sources tracked · "
    ...
    private var rankings: some View {
        HStack(alignment: .top, spacing: 46) {
            VStack(alignment: .leading, spacing: 6) {
                self.sectionHeader("COST SOURCES", detail: "\(self.payload.providers.count) WITH HISTORY")
                ForEach(
