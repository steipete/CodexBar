...
struct CostHistoryChartMenuView: View {
    ...
    private static func makeModel(provider: UsageProvider, daily: [DailyEntry]) -> Model {
        ...
        var maxDetailRowsHeight = CGFloat(0)
        for entry in sorted {
            ...
            maxDetailRowsHeight = max(maxDetailRowsHeight, Self.renderedBreakdownRowsHeight(for: entry))
            if let cur = peak {
    ...
    private static func renderedBreakdownRowsHeight(for entry: DailyEntry) -> CGFloat {
        guard let breakdown = entry.modelBreakdowns, !breakdown.isEmpty else { return 0 }
