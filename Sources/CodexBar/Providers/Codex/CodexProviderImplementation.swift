...
struct CodexProviderImplementation: ProviderImplementation {
    ...
    func appendUsageMenuEntries(context: ProviderMenuUsageContext, entries: inout [ProviderMenuEntry]) {
        ...
        if let credits = context.store.credits {
            entries.append(.text(String(format: L("credits_remaining"), UsageFormatter.creditsString(from: credits.remaining)), .primary))
            if let latest = credits.events.first {
                entries.append(.text(String(format: L("last_spend"), UsageFormatter.creditEventSummary(latest)), .secondary))
            }
        } else {
