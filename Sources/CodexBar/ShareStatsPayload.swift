...
struct ShareStatsPayload: Sendable, Equatable {
    ...
    var tokenCoverageIsComplete: Bool {
        self.totalTokens != nil
            && self.tokenSourceCount == self.providers.count
            && self.providers.allSatisfy { $0.coveredDayCount >= self.days }
    }
    ...
    var hasModelActivityData: Bool {
        self.providers.contains { $0.totalTokens != nil }
    }

    var hasShareableData: Bool {
        !self.providers.isEmpty && self.providers.contains {
            $0.totalTokens != nil || $0.estimatedCost != nil
        }
    }
}
...
enum ShareStatsSanitizer {
    static func modelName(_ rawValue: String) -> String? {
        ...
        if let suffix = self.suffix(in: familyName, after: ["anthropic.claude-", "claude-", "claude "]),
           let canonical = self.canonicalSuffix(
               suffix,
               allowedStarts: [
                   "fable", "opus", "sonnet", "haiku", "instant",
                   "1", "2", "3", "4", "5", "6", "7", "8", "9",
               ])
        {
    ...
    private static func canonicalSuffix(_ value: String, allowedStarts: [String]) -> String? {
        ...
        let publicQualifiers: Set = [
            "a", "air", "canvas", "chat", "codex", "coder", "deep", "fable", "flash", "haiku",
            "image", "instant", "instruct", "large", "lite", "luna", "max", "medium", "micro",
            "mini", "nano", "nightly", "opus", "plus", "premier", "preview", "pro", "r",
            "reasoning", "reel", "small", "sol", "sonic", "sonnet", "spark", "speech", "terra",
    ...
    private static func prettySuffix(_ value: String) -> String {
        value
            ...
            .map { part in
                ...
                if ["pro", "sol"].contains(value.lowercased()) {
        ...
        let payload = ShareStatsPayload(
            ...
            modelRouteCoverageIsComplete: model.groups.allSatisfy {
                $0.modelTokenHistoryCompleteness == .complete
                    && $0.providers.allSatisfy { $0.coveredDayCount >= model.requestedDays }
            },
            totalTokens: totalTokens,
            tokenSourceCount: knownTokenTotals.count,
            trackedSourceCount: trackedSources.isEmpty ? providers.count : trackedSources.count)
...
enum ShareStatsFormatting {
    ...
    private static func modelActivityText(_ payload: ShareStatsPayload) -> String {
        ...
        if payload.hiddenModelRouteCount > 0 {
            let routeLabel = payload.hiddenModelRouteCount == 1 ? "route name" : "route names"
            lines.append("\(payload.hiddenModelRouteCount) private \(routeLabel) omitted")
        }
        if !payload.modelRouteCoverageIsComplete {
            lines.append("Model route history is partial")
        }
        lines.append("\(payload.trackedSourceCount) sources tracked")
        if payload.trackedSourceCount > payload.providers.count {
            lines.append(
                "\(payload.providers.count) with cost history · "
                    + "\(payload.trackedSourceCount - payload.providers.count) excluded from cost totals")
        }
        lines.append(
