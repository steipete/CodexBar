...
struct CreditsHistoryChartMenuView: View {
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 10) {
            if model.points.isEmpty {
                Text(AppStrings.tr("No credits history data."))
                    .font(.footnote)
    ...
    private func detailLines(model: Model) -> (primary: String, secondary: String?) {
        ...
        if day.services.isEmpty {
            return (AppStrings.fmt("%@: %@ credits", dayLabel, total), nil)
        }
        if day.services.count <= 1, let first = day.services.first {
            ...
            return (AppStrings.fmt("%@: %@ credits", dayLabel, used), first.service)
        }
        ...
        return (AppStrings.fmt("%@: %@ credits", dayLabel, total), services)
    }
}
