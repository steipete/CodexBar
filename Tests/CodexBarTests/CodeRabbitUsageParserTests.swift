...
struct CodeRabbitUsageParserTests {
    ...
    @Test
    func `coderabbit cli probe executes usage script and parses output`() async throws {
        let script = """
        if [ "$1" = "usage" ]; then
            cat <<'EOF'
        ────────────────────────────────────────
        ...
        let now = Date(timeIntervalSince1970: 1_700_000_000)
        let probe = CodeRabbitCLIProbe(
            usageArguments: ["-c", script, "coderabbit", "usage"],
            authStatusArguments: ["-c", script, "coderabbit", "auth", "status"])

        let snapshot = try await probe.fetch(
            environment: ["CODERABBIT_CLI_PATH": "/bin/sh"],
            now: now)

        #expect(snapshot.organization == "TestOrg")
        #expect(snapshot.user == "TestUser")
        #expect(snapshot.reviewsCount == 10)
        #expect(snapshot.accountEmail == "test@example.com")
        #expect(snapshot.plan == "Team")
        #expect(snapshot.periodResetsRaw == "2026-11-01")
    }
}
