...
struct TypeSafePluginTests {
    ...
    @Test(arguments: BundledPluginTestSupport.engines)
    func `action discovery stops after the first matching chunk`(engine: ProviderPluginEngineKind) async throws {
        let requests = RequestLog()
        let html = Self.html + #"<script src="/_next/static/chunks/later.js"></script>"#
        _ = try await Self.fetch(engine: engine) { request in
            await requests.append(request)
            return try await Self.defaultHandler(html: html, chunk: Self.chunk, body: Self.body)(request)
        }
        #expect(await requests.all.contains { $0.url?.path.hasSuffix("later.js") == true } == false)
    }

    @Test(arguments: [22, 23, 24, 300], [false, true])
    func `large credit lists remain within detail limits without changing reported money`(
        count: Int, hasPlan: Bool) async throws
    {
        for engine in BundledPluginTestSupport.engines {
            let credit: [String: Any] = ["amount": 5, "remaining": 4, "expiresAt": "2027-01-01T00:00:00Z"]
            var billing: [String: Any] = [
                "spent": 12.5,
                "balance": 99,
                "credits": Array(repeating: credit, count: count) + [
                    ["amount": 5, "remaining": 0, "expiresAt": "2027-01-01T00:00:00Z"],
                    ["amount": 5, "remaining": 4, "expiresAt": "invalid"],
                ],
            ]
            if hasPlan { billing["plan"] = "free_plan" }
            let json = try JSONSerialization.data(withJSONObject: ["ok": true, "data": ["billing": billing]])
            let body = try "1:" + #require(String(data: json, encoding: .utf8))
            let snapshot = try await Self.fetch(
                engine: engine, handler: Self.defaultHandler(html: Self.html, chunk: Self.chunk, body: body))
            #expect(snapshot.providerCost?.used == 12.5)
            #expect(snapshot.providerCost?.balance == 99)
            let rows = try #require(snapshot.details.first?.rows)
            #expect(rows.count <= 24)
            let slots = hasPlan ? 22 : 23
            if count > slots {
    ...
    @Test(arguments: [429, 503], BundledPluginTestSupport.engines)
    func `chunk outages retain transient classification and retry delay`(
        status: Int, engine: ProviderPluginEngineKind) async throws
    {
        do {
            _ = try await Self.fetch(engine: engine) { request in
                if request.url?.path.hasSuffix("app.js") == true {
                    return try Self.response(
                        request,
                        status: status,
            ...
            Issue.record("Expected a classified discovery failure")
    ...
    @Test(arguments: [404, 503], BundledPluginTestSupport.engines)
    func `a later matching chunk survives an unrelated missing or unavailable chunk`(
        status: Int, engine: ProviderPluginEngineKind) async throws
    {
        let html = Self.html + #"<script src="/_next/static/chunks/billing.js"></script>"#
        let snapshot = try await Self.fetch(engine: engine) { request in
            if request.url?.path.hasSuffix("app.js") == true {
                return try Self.response(request, status: status, body: "unavailable")
            }
            if request.url?.path.hasSuffix("billing.js") == true {
                return try Self.response(request, body: Self.chunk)
            }
            return try await Self.defaultHandler(html: html, chunk: Self.chunk, body: Self.body)(request)
        }
        #expect(snapshot.providerCost?.balance == 4.98)
    }
