...
struct CursorStatusProbeTests {
    ...
    func `enterprise overall drives headline percent and dollars`() throws {
        ...
        // Allow a tiny tolerance for floating-point division (7384/10000 * 100 ≈ 73.83999…).
        #expect(abs(snapshot.planPercentUsed - 73.84) < 0.0001)
        #expect(snapshot.planUsedUSD == 73.84)
        #expect(snapshot.planLimitUSD == 100.0)
        // Lane percents stay nil because Cursor doesn't ship Auto/API breakdown for Enterprise overall.
        #expect(snapshot.autoPercentUsed == nil)
        #expect(snapshot.apiPercentUsed == nil)
        // Headline must NOT pick up the team pool's 45.25% — personal cap wins.
    ...
    func `enterprise pooled fallback used when no individual data`() {
        ...
        // 12_725_135 / 28_122_000 ≈ 45.2497...% — accept anything in (45.0, 45.5) to keep the
        // assertion robust to floating-point precision.
    ...
    func `existing plan block still wins over overall and pooled`() {
        ...
        // `plan.totalPercentUsed` wins; `overall` and `pooled` are ignored when `plan` is present.
