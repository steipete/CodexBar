...
struct PiSessionProcessContextTests {
        ...
        let roots = PiFamilySessionScanner.costSessionRoots(
            ...
            processContexts: [
                PiSessionProcessContext(
                    ...
                    workingDirectory: project,
                    selectorEnvironment: environment),
            ])
        ...
        let shared = try PiSessionCostScanner.loadDailyReportResultCancellable(
            ...
            options: PiSessionCostScanner.Options(
                ...
                processContexts: [project, secondProject].map {
                    PiSessionProcessContext(command: "pi", workingDirectory: $0, selectorEnvironment: environment)
                }), checkCancellation: nil)
    ...
    @Test(arguments: [false, true], ["invalidJSON", "missingProject", "danglingSettingsDirectory"])
    func `pi cost cache preserves the previous report when retained settings are unavailable`(
        usesDefaultRoot: Bool, failure: String) throws
    {
        let env = try CostUsageTestEnvironment()
        defer { env.cleanup() }
        ...
        let parked = env.root.appendingPathComponent("unavailable-project-backup", isDirectory: true)
        switch failure {
        case "missingProject":
            try FileManager.default.moveItem(at: project, to: parked)
        case "danglingSettingsDirectory":
            try FileManager.default.moveItem(at: settingsURL.deletingLastPathComponent(), to: parked)
            try FileManager.default.createSymbolicLink(
                at: settingsURL.deletingLastPathComponent(),
                withDestinationURL: env.root.appendingPathComponent("offline-volume", isDirectory: true))
        default:
        ...
        #expect(afterFailure.lastScanAt == first.lastScanAt)
        ...
        switch failure {
        case "missingProject":
            try FileManager.default.moveItem(at: parked, to: project)
        case "danglingSettingsDirectory":
            try FileManager.default.removeItem(at: settingsURL.deletingLastPathComponent())
            try FileManager.default.moveItem(at: parked, to: settingsURL.deletingLastPathComponent())
        default:
            try savedSettings.write(to: settingsURL, options: .atomic)
        }
        let restored = try PiSessionCostScanner.loadDailyReportResultCancellable(
            provider: .codex,
            since: day,
            until: day,
        ...
        #expect(restored.report.summary?.totalTokens == 9)
        #expect(restored.lastScanAt == day.addingTimeInterval(2))
    }

    @Test
    func `a relative Pi selector with unavailable cwd keeps history incomplete`() async throws {
        let env = try CostUsageTestEnvironment()
        defer { env.cleanup() }
        let environment = ["HOME": env.root.path]
        let scanner = LocalAgentSessionScanner(
            processOutputProvider: { _ in
                "201 1 Mon Jul 6 09:03:00 2026 /usr/local/bin/pi --session-dir ./sessions"
            },
            cwdProvider: { _, _ in [:] },
            processEnvironmentProvider: { _ in [201: environment] })
        let contexts = await scanner.piSessionProcessContexts(environment: environment)
        #expect(contexts.count == 1)
        #expect(contexts.first?.workingDirectory == nil)
    ...
    func `pi process context cap preserves a distinct older session root`() async throws {
        ...
        let scanner = LocalAgentSessionScanner(
            ...
            processEnvironmentProvider: { pids in
                Dictionary(uniqueKeysWithValues: pids.map { ($0, environment) })
            })
