...
struct ClaudeUsageTests {
    ...
    func `parses claude web API usage response`() throws {
        let json = """
        {
          ...
          "seven_day_design": { "utilization": 31, "resets_at": "2025-12-30T23:00:00.000Z" },
          "seven_day_routines": { "utilization": 7, "resets_at": "2025-12-31T23:00:00.000Z" }
        }
        """
        let data = Data(json.utf8)
        ...
        #expect(parsed.extraRateWindows.first(where: { $0.id == "claude-design" })?.window.usedPercent == 31)
        #expect(parsed.extraRateWindows.first(where: { $0.id == "claude-routines" })?.window.usedPercent == 7)
