...
struct CopilotBudgetWebFetcherTests {
    ...
    // A unit test on the row builder proves the mapping itself is correct, but not that
    // `addOrgCreditsIfNeeded` calls it with the right arguments, in the right order, or at all.
    ...
    func `org credits merge onto the seat lane through the fetch strategy`() async {
        ...
        let seatRow = result.usage.detailRow(id: CopilotCreditDetailRows.seatRowID)
        #expect(seatRow?.value == "31 / 3000")
        #expect(seatRow?.progress?.total == 3000)
        // Org row comes from the billing response, denominated by orgCreditEntitlement -- NOT
        // seatCreditEntitlement. A swap of the two settings fields would flip this to 3000.
        let orgRow = result.usage.detailRow(id: CopilotCreditDetailRows.orgRowID)
        #expect(orgRow?.label == "Org credits (example-org)")
        #expect(orgRow?.value == "81.1 / 6000")
        #expect(orgRow?.progress?.used == 81.1)
        #expect(orgRow?.progress?.total == 6000)
        // Both lanes live in the same shared Credits section, and the org row carries the seat
        // payload's reset date rather than a nil or independently derived one.
        #expect(result.usage.detailRow(id: CopilotCreditDetailRows.seatRowID) != nil)
        #expect(orgRow?.secondaryValue != nil)
        #expect(orgRow?.secondaryValue == seatRow?.secondaryValue)
    }
