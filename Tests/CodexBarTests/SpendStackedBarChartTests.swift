...
struct SpendStackedBarChartTests {
    ...
    @Test
    func `tied stackEnd values resolve to exactly one id`() {
        let day = Date(timeIntervalSince1970: 0)
        let a = Self.dailyPoint(sourceID: "claude", day: day, stackStart: 0, stackEnd: 10)
        let b = Self.dailyPoint(sourceID: "codex", day: day, stackStart: 0, stackEnd: 10)
        ...
        #expect(topIDs.count == 1)
        #expect(topIDs.isSubset(of: [a.id, b.id]))
    }
