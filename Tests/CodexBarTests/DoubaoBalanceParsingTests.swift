...
struct DoubaoBalanceParsingTests {
    @Test
    func `parses standard balance response`() throws {
        ...
        """
        let info = try DoubaoUsageFetcher._parseBalanceForTesting(Data(json.utf8))
    ...
    func `invalid JSON throws parse error`() {
        ...
        #expect {
            _ = try DoubaoUsageFetcher._parseBalanceForTesting(data)
        } throws: { error in
