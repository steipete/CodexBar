...
struct ShareStatsTests {
    ...
    func `builder preserves native currencies and unavailable spend`() throws {
        ...
        let payload = try #require(ShareStatsBuilder.make(
            ...
            trackedSources: [
                Self.trackedSource(id: "codex:one", provider: .codex, contributes: true),
                Self.trackedSource(id: "claude", provider: .claude, contributes: true),
                Self.trackedSource(id: "cursor", provider: .cursor, contributes: true),
                Self.trackedSource(id: "openrouter:one", provider: .openrouter, contributes: false),
            ]))

        #expect(payload.days == 30)
        #expect(payload.trackedSourceCount == 4)
        ...
        #expect(text.contains("4 sources tracked"))
        #expect(text.contains("3 with cost history · 1 excluded from cost totals"))
    ...
    @Test
    func `text reports private routes when every route name is hidden`() throws {
        let group = SpendDashboardModel.CurrencyGroup(
            ...
            models: [
                SpendDashboardModel.ModelRow(
                    ...
                    modelName: "acme/private-model-v2",
                    totalTokens: 10,
                    totalCost: 1),
            ],
            dailyPoints: [],
        ...
        #expect(payload.hiddenModelRouteCount == 1)
        let text = ShareStatsFormatting.text(payload, style: .modelActivity)
        #expect(text.contains("1 private route name omitted"))
        #expect(!text.contains("acme"))
    }
    ...
    @Test
    func `latest priced model variants keep their public identities`() {
        #expect(ShareStatsSanitizer.modelName("gpt-5.6-sol") == "GPT-5.6 Sol")
        #expect(ShareStatsSanitizer.modelName("gpt-5.6-terra") == "GPT-5.6 Terra")
        #expect(ShareStatsSanitizer.modelName("gpt-5.6-luna") == "GPT-5.6 Luna")
        #expect(ShareStatsSanitizer.modelName("gpt-5.3-codex-spark") == "GPT-5.3 Codex Spark")
        #expect(ShareStatsSanitizer.modelName("claude-fable-5") == "Claude Fable 5")
        #expect(ShareStatsSanitizer.modelName("claude-4.5-sonnet") == "Claude 4.5 Sonnet")
    }
    ...
    @Test
    func `cost only dashboard keeps the summary share payload`() throws {
        let group = SpendDashboardModel.CurrencyGroup(
            currencyCode: "USD",
            providers: [
                SpendDashboardModel.ProviderRow(
        ...
        let payload = try #require(ShareStatsBuilder.make(
            model: SpendDashboardModel(requestedDays: 7, groups: [group])))
        #expect(payload.hasShareableData)
        #expect(!payload.hasModelActivityData)
        #expect(ShareStatsFormatting.text(payload).contains("USD: $4.00 estimated"))
    }
    ...
    func `seven of thirty covered days stay partial`() throws {
        ...
        #expect(!payload.modelRouteCoverageIsComplete)
        let text = ShareStatsFormatting.text(payload, style: .modelActivity)
        #expect(text.contains("at least 10 tracked tokens"))
        #expect(text.contains("at least 1 of 30 days active"))
        #expect(text.contains("Model route history is partial"))
    }
    ...
    @Test
    func `activity strip groups every supported range into complete weeks`() {
        #expect(ShareStatsModelActivityCardView.weekCount(for: 0) == 0)
        #expect(ShareStatsModelActivityCardView.weekCount(for: 7) == 1)
        #expect(ShareStatsModelActivityCardView.weekCount(for: 30) == 5)
        #expect(ShareStatsModelActivityCardView.weekCount(for: 365) == 53)
    }
...
extension ShareStatsTests {
    @Test
    func `eight token cost providers and model routes are preserved`() throws {
        let sources = [
            ProofSource(
                id: "codex",
                provider: .codex,
        ...
        let group = SpendDashboardModel.CurrencyGroup(
            ...
            dailyPoints: [],
            dailyTokenPoints: sources.map { source in
                SpendDashboardModel.DailyTokenPoint(
                    sourceID: source.id,
                    provider: source.provider,
                    providerName: source.name,
        ...
        #expect(payload.topModels.count == 8)
        #expect(payload.shareableModelRouteCount == 8)
        #expect(payload.hiddenModelRouteCount == 0)
        #expect(payload.tokenCoverageIsComplete)
        #expect(payload.modelRouteCoverageIsComplete)
        #expect(Set(payload.topModels.map(\.modelName)) == [
    ...
    private static func trackedSource(
        id: String,
        provider: UsageProvider,
        contributes: Bool) -> SpendDashboardTrackedSource
    {
        SpendDashboardTrackedSource(
            ...
            accountName: nil,
            state: .connected,
            supportsCostHistory: contributes,
            contributesCostHistory: contributes)
    }
