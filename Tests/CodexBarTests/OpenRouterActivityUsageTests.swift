...
struct OpenRouterActivityUsageTests {
    ...
    @Test
    func `activity fetch requests the thirty completed day endpoint`() async throws {
        ...
        let activity = try #require(try await OpenRouterActivityUsageFetcher.fetchOptional(
            apiKey: "fixture-management-key",
            baseURL: #require(URL(string: "https://openrouter.test/api/v1")),
            transport: transport,
            now: Self.now,
            historyDays: 30))

        let request = try #require(await requests.requests.only)
        #expect(request.httpMethod == "GET")
        #expect(request.url?.absoluteString == "https://openrouter.test/api/v1/activity")
        #expect(request.value(forHTTPHeaderField: "Authorization") == "Bearer fixture-management-key")
        #expect(activity.toCostUsageTokenSnapshot().historyDays == 30)
    }

    @Test(arguments: [403, 404])
    func `unavailable activity is optional and never fabricates rows`(statusCode: Int) async throws {
        let transport = ProviderHTTPTransportHandler { request in
            try Self.response(
                request,
                body: #"{"error":{"message":"activity unavailable"}}"#,
                statusCode: statusCode)
        }

        let activity = try await OpenRouterActivityUsageFetcher.fetchOptional(
            apiKey: "fixture-key",
            baseURL: #require(URL(string: "https://openrouter.test/api/v1")),
            transport: transport,
            now: Self.now)
    ...
    @Test
    func `malformed optional activity never fabricates rows`() async throws {
        let transport = ProviderHTTPTransportHandler { request in
            try Self.response(request, body: #"{"data":[{"date":"2026-08-06","usage":"free"}]}"#)
        }
    ...
    #if !canImport(JavaScriptCore)
    @Test
    func `provider snapshot exposes optional activity cost history`() throws {
        let activity = try OpenRouterActivityUsageSnapshot(
            data: Data(Self.activityFixture.utf8),
            now: Self.now)
        let provider = OpenRouterUsageSnapshot(
            totalCredits: 100,
            totalUsage: 40,
            balance: 60,
            usedPercent: 40,
        ...
        #expect(provider.activityUsage?.toCostUsageTokenSnapshot() == activity.toCostUsageTokenSnapshot())
        #expect(provider.toCostUsageTokenSnapshot() == activity.toCostUsageTokenSnapshot())
    }
    #endif
