...
struct ProviderArchitectureGatekeeperTests {
    ...
    func `small provider capabilities preserve legacy registries`() {
        ...
        #expect(Set(descriptors.filter(\.tokenCost.supportsTokenSnapshot).map(\.id)) == [
            .codex, .claude, .cursor, .vertexai, .bedrock, .gemini, .grok,
        ])
        #else
        #expect(Set(descriptors.filter(\.tokenCost.supportsTokenSnapshot).map(\.id)) == [
            .codex, .claude, .vertexai, .bedrock, .gemini, .grok,
        ])
        #endif
    ...
    private static let allowedProviderConstructs: [AllowedProviderConstruct] = [
        ...
        AllowedProviderConstruct(
            ...
            reason: "Derived cross-provider construct cluster."),
        AllowedProviderConstruct(
        ...
        AllowedProviderConstruct(
            ...
            expectedProviderIDs: ["codex"],
            expectedReferenceCount: 6,
            expectedReferenceFingerprint: ["codex@0", "codex@1", "codex@3", "codex@6", "codex@7", "codex@9"],
            reason: "This exact app-runtime bridge coordinates provider-owned state through the shared controller."),
        AllowedProviderConstruct(
            path: "Sources/CodexBar/UsageStore+TokenAccounts.swift",
            line: 1451,
        ...
        AllowedProviderConstruct(
            ...
            expectedProviderIDs: ["claude"],
            expectedReferenceCount: 3,
            expectedReferenceFingerprint: ["claude@0", "claude@5", "claude@15"],
        ...
        AllowedProviderConstruct(
            path: "Sources/CodexBarCore/CostUsageFetcher.swift",
            line: 1459,
            anchor: "let shouldMergePiUsage = provider != .codex || scopedCodexHomePath?.isEmpty != false",
            expectedProviderIDs: ["claude", "codex", "gemini", "grok", "openai", "vertexai", "xai"],
        ...
        AllowedProviderConstruct(
            ...
            expectedProviderIDs: ["openai"],
            expectedReferenceCount: 2,
            expectedReferenceFingerprint: ["openai@0", "openai@9"],
            reason: "Derived cross-provider construct cluster."),
        AllowedProviderConstruct(
        ...
        AllowedProviderConstruct(
            ...
            expectedProviderIDs: ["claude"],
            expectedReferenceCount: 2,
            expectedReferenceFingerprint: ["claude@0", "claude@11"],
            reason: "Derived cross-provider construct cluster."),
        AllowedProviderConstruct(
        ...
        AllowedProviderConstruct(
            path: "Sources/CodexBar/UsageStore+CodexCostCatchUp.swift",
            line: 266,
            anchor: "self.lastTokenFetchScope[.codex] = context.scopeSignature",
            expectedProviderIDs: ["codex"],
            expectedReferenceCount: 5,
            expectedReferenceFingerprint: ["codex@0", "codex@3", "codex@3", "codex@6", "codex@8"],
            reason: "Provider-specific by design: cross-provider dispatch in shared code."),
        AllowedProviderConstruct(
            path: "Sources/CodexBar/UsageStore+CodexCostCatchUp.swift",
            line: 293,
            anchor: "&& self.tokenCostScope(for: .codex).codexHomePath == context.codexHomePath",
            expectedProviderIDs: ["codex"],
            expectedReferenceCount: 2,
            expectedReferenceFingerprint: ["codex@0", "codex@1"],
            reason: "Provider-specific by design: cross-provider dispatch in shared code."),
