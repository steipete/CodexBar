...
struct MenuCardOverrideIsolationTests {
    ...
    @Test
    func `active account card can include provider adjunct data explicitly`() throws {
        let suite = "MenuCardOverrideIsolationTests-active-\(UUID().uuidString)"
        let defaults = try #require(UserDefaults(suiteName: suite))
        defaults.removePersistentDomain(forName: suite)
        let settings = SettingsStore(
            userDefaults: defaults,
        ...
        let model = try #require(controller.menuCardModel(
            ...
            forceOverrideCard: true,
            accountOverride: AccountInfo(email: "account@example.com", plan: nil),
            allowsProviderDataForAccountOverride: true))
