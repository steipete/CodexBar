...
struct StatusItemAnimationTests {
    ...
    func `menu bar display text uses open router balance`() {
        let settings = SettingsStore(
            configStore: testConfigStore(suiteName: "StatusItemAnimationTests-openrouter-balance"),
            zaiTokenStore: NoopZaiTokenStore())
        settings.statusChecksEnabled = false
        settings.refreshFrequency = .manual
        settings.mergeIcons = true
    ...
    func `menu bar display text uses deepseek balance`() {
        let settings = SettingsStore(
            configStore: testConfigStore(suiteName: "StatusItemAnimationTests-deepseek-balance"),
            zaiTokenStore: NoopZaiTokenStore(),
            syntheticTokenStore: NoopSyntheticTokenStore())
        settings.statusChecksEnabled = false
        settings.refreshFrequency = .manual
        settings.mergeIcons = true
        settings.selectedMenuProvider = .deepseek
        settings.menuBarDisplayMode = .both
        settings.usageBarsShowUsed = true

        let registry = ProviderRegistry.shared
        if let deepSeekMeta = registry.metadata[.deepseek] {
            settings.setProviderEnabled(provider: .deepseek, metadata: deepSeekMeta, enabled: true)
        }
